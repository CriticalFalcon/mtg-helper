...
export class CardSearchComponent {
	...
	searchCard() {
		...
		this.http
			.get<any>(`https://api.scryfall.com/cards/named?fuzzy=${encodeURIComponent(this.searchTerm)}`)
			.subscribe({
				next: (data) => {
					this.cardImage =
						data.image_uris?.normal || (data.card_faces ? data.card_faces[0].image_uris.normal : null);
						...
						this.http.get<any>(data.prints_search_uri).subscribe({
							next: (printsData) => {
								...
								this.prints = rawPrints.map((p) => ({
									setName:
										nameCounts[p.setName] > 1 ? `${p.setName} #${p.collectorNumber}` : p.setName,
									imageUrl: p.imageUrl,
								}));
							},
							error: () => (this.prints = []),
